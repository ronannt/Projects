...
class Funcs:
    ...
    def TipoPessoa(radio_fisica, radio_juridica, radio_10kw, radio_75kw, home, tp_proj):
        global docprocuracao
        global wb
        global fmtp
        pstp = 0
        fmtp = 0
        if radio_fisica.isChecked():
            docprocuracao = Document(f"{myDir}/Forms/Procuracao_pf.docx")
            pstp = 1
            home.lineempresa.hide()
            home.linecnpj.hide()
        elif radio_juridica.isChecked():
            docprocuracao = Document(f"{myDir}/Forms/Procuracao_pj.docx")
            pstp = 2
            home.lineempresa.show()
            home.linecnpj.show()
        else:
            tp_proj.label_erro.setText("Selecione o tipo de pessoa!")
            tp_proj.frame_erro.show()
            pstp = 0

        if radio_10kw.isChecked():
            wb = load_workbook(f"{myDir}/Forms/formularioEXL.xlsx")
            fmtp = 1
            home.listpot.show()
            home.listpot2.hide()
        elif radio_75kw.isChecked():
            """Mudar o formulario para """
            wb = load_workbook(f"{myDir}/Forms/formularioEXL.xlsx")
            fmtp = 2
            home.listpot.hide()
            home.listpot2.show()
        else:
            tp_proj.label_erro.setText("Selecione a potencia!")
            tp_proj.frame_erro.show()
            fmtp = 0

        if pstp == 1 and fmtp == 1:
            home.show()
            tp_proj.close()
        elif pstp == 2 and fmtp == 1:
            home.show()
            tp_proj.close()
        elif pstp == 2 and fmtp == 2:
            home.show()
            tp_proj.close()
        elif pstp == 1 and fmtp == 2:
            home.show()
            tp_proj.close()       
        else:
            tp_proj.label_erro.setText("Selecione duas opções!")
            tp_proj.frame_erro.show()
    ...
    linecnpj):
            ...
            if fmtp == 1:
                potf = pot.currentItem().text()
            elif fmtp == 2:
                potf = pot2.currentItem().text()
            ...
            lines = [home.linetituc,
                    ...
                    home.linefabmod,
                    home.linemodmod,
                    home.lineareaarranjo,
                    home.lineqtdinversor,
                    home.linefabinversor,
                    home.linemodinversor,
                    home.linesomamod,
                    home.linesomainversor,
                    home.linedataprev,
                    home.linelttd,
                    home.linelgtd,
                    home.linecargainsta,
                    home.linecargainstagera,
                    home.linetensao,
                    home.lineptnmod,
                    home.lineqtdarranjo,
                    home.lineseriearranjo,
                    home.lineptninversor,
                    home.lineqtdentinversor,
                    home.linedisjuntor2,
                    home.lineespcc,
                    home.lineespca,
                    home.linetpatvd]
